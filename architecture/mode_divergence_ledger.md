...
| DIV-006 | accounting | paper, sandbox, live | AccountState diagnostics now support read-through plus optional mirror-write parity checks; broker internals remain primary write path by default. | yes | Phase 3 staged migration enables mirror-write and strict test policies without making AccountState globally write-authoritative yet. | tests/contracts/state/test_account_state_contracts.py | medium | verified |
