...
| DIV-006 | accounting | paper, sandbox, live | AccountState read-through mismatch diagnostics are available, but broker internals remain write-authoritative during transition. | yes | Phase 3 staged migration uses compare-only diagnostics before canonical AccountState writes. | tests/contracts/state/test_account_state_contracts.py | medium | open |
