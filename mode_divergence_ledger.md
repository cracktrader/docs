...
| MDL-003 | fill timestamp source | backtest vs paper/sandbox/live | Fill events without explicit timestamps are rejected in `backtest`, but permitted in `paper`/`sandbox`/`live` via clock fallback. | Backtest determinism requires explicit data-time provenance; real-time modes prioritize continuity when upstream events omit timestamps. | engine team | active | #40 | `tests/contracts/test_mode_contracts.py`, `tests/unit/engine/test_events.py` | Fill timing semantics are mode-governed and should not silently converge across all modes. |
